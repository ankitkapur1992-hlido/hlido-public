...
"""Hlido function group for NVIDIA NeMo Agent Toolkit (optional ``nemo`` extra).

This module is the entry-point target for the ``nat.plugins`` group. Importing it
runs ``@register_function_group`` so the NeMo Agent Toolkit discovers the
``hlido`` function group, letting a workflow vet **another** agent before
delegating to, installing, or relying on it.

Install with::

    pip install "hlido[nemo]"

Generated tool names (default ``hlido`` instance):
    hlido__trust_check / hlido__find_trusted / hlido__verify_claim / hlido__get_scorecard

Requires ``nvidia-nat-core``; importing without it raises a clear error.
"""
...
try:
    ...
    raise ImportError(
        "hlido_trust.nat_plugin requires the NeMo Agent Toolkit. "
        'Install it with: pip install "hlido[nemo]"'
    ) from exc
